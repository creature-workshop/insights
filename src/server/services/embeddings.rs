...
impl EmbeddingModel {
    ...
    fn add_past_key_values(
        ...
        let past_key_names: Vec<&String> = model_input_names
            .iter()
    ...
    fn extract_embedding(output: &dyn EmbeddingOutput) -> Result<Vec<f32>> {
        let tensor = output
            ...
            .ok_or_else(|| {
                anyhow!("No output found from model - expected 'last_hidden_state' or '0'")
            })?;
...
async fn create_embedding_with_prompt(formatted_text: &str) -> Result<Vec<f32>> {
    ...
    let needs_init = {
        let guard = mutex
            .lock()
            .map_err(|_| anyhow!("Failed to lock model mutex"))?;
        guard.is_none()
    };
...
mod gte_base_tests {
    ...
    fn test_cosine_similarity() {
        ...
        assert!(
            opposite_sim < 0.001,
            "Opposite vectors should have similarity near 0.0"
        );

...
async fn attempt_embedding_deletion(context: &RequestContext, request: &RemoveInsightRequest) {
    match context
        .vector_db
        .delete_embedding(&request.topic, &request.name)
        .await
    {
        Ok(_) => {
...
async fn log_embedding_deletion_success(context: &RequestContext, request: &RemoveInsightRequest) {
    context
        .log_success(
            &format!(
                "Successfully deleted insight {}/{} and its embedding",
...
fn create_insight_removal_error(
    ...
    let api_error = ApiError::new(
        "insight_remove_failed",
        &format!("Failed to remove insight: {error}"),
    );
    (
        axum::http::StatusCode::INTERNAL_SERVER_ERROR,
        ResponseJson(BaseResponse::<()>::error(vec![api_error], transaction_id)),
...
) -> Result<ResponseJson<BaseResponse<()>>, (axum::http::StatusCode, ResponseJson<BaseResponse<()>>)>
{
    ...
    context
        .log_info("Starting insight re-indexing process", "insights-api")
        .await;
    ...
    tokio::spawn(async move {
        if let Err(e) = perform_reindexing(context.clone()).await {
            context
                .log_error(&format!("Re-indexing failed: {e}"), "insights-api")
                .await;
        }
    });
...
async fn load_all_insights_for_reindexing(
    ...
    context
        .log_info("Loading all insights for re-indexing", "insights-reindex")
        .await;
    ...
    context
        .log_info(
            &format!("Found {total_insights} insights to re-index"),
            "insights-reindex",
        )
        .await;
...
async fn clear_existing_embeddings(context: &RequestContext) -> Result<()> {
    context
        .log_info(
            "Starting clean slate database recreation",
            "insights-reindex",
        )
        .await;
    ...
    let embedding_dimension =
        match crate::server::services::embeddings::detect_embedding_dimension().await {
            Ok(dim) => {
                context
                    .log_info(
                        &format!("Detected embedding model dimension: {dim}"),
                        "insights-reindex",
                    )
                    .await;
                dim
    ...
    context
        .vector_db
        .reshape_database(embedding_dimension)
        .await?;
    context
        .log_info("Database reshape completed", "insights-reindex")
        .await;
...
async fn clear_existing_embeddings(context: &RequestContext) -> Result<()> {
    context
        .log_info(
            "Skipping embedding clearing (no ML features)",
            "insights-reindex",
        )
        .await;
    Ok(())
...
async fn generate_and_store_embedding(
    ...
    context
        .vector_db
        .store_embedding(&insight_with_embedding)
        .await?;
...
async fn log_insight_addition_start(context: &RequestContext, request: &AddInsightRequest) {
    context
        .log_info(
            &format!("Adding insight {}/{}", request.topic, request.name),
            "insights-api",
        )
        .await;
}
...
async fn log_embedding_success(context: &RequestContext, insight: &insight::Insight) {
    context
        .log_success(
            &format!(
                "Successfully added insight {}/{} with embedding",
                insight.topic, insight.name
            ),
            "insights-api",
        )
...
fn create_insight_save_error(
    ...
    let api_error = ApiError::new(
        "insight_add_failed",
        &format!("Failed to add insight: {error}"),
    );
    (
        axum::http::StatusCode::INTERNAL_SERVER_ERROR,
        ResponseJson(BaseResponse::<()>::error(vec![api_error], transaction_id)),
...
> {
    ...
    context
        .log_info(
            &format!("Retrieving insight {}/{}", request.topic, request.name),
            "insights-api",
        )
        .await;
    ...
    match insight::load(&request.topic, &request.name) {
        Ok(mut insight_data) => {
            ...
            let insight = InsightData {
                ...
                details: if request.overview_only {
                    String::new()
                } else {
...
async fn log_search_start(context: &RequestContext, request: &SearchRequest) {
    context
        .log_info(
            &format!(
                "Searching insights: terms={:?}, topic={:?}",
                request.terms, request.topic
            ),
            "insights-api",
        )
        .await;
...
async fn log_embedding_search_failure(context: &RequestContext, terms: &[String]) {
    context
        .log_error(
            &format!("Embedding search failed for {terms:?}"),
            "insights-api",
        )
        .await;
}
    ...
    match context.vector_db.has_embeddings().await {
        Ok(true) => {
            context
                .log_info(
                    &format!(
                        "Starting embedding search for {:?} (embeddings exist)",
                        request.terms
                    ),
                    "insights-api",
                )
        ...
        Ok(false) => {
            context
                .log_info(
                    &format!(
                        "Skipping embedding search for {:?} (no embeddings in database)",
                        request.terms
                    ),
                    "insights-api",
                )
        ...
        Err(e) => {
            context
                .log_error(
                    &format!("Failed to check for embeddings: {e}"),
                    "insights-api",
                )
                .await;
...
async fn check_embeddings_availability(
    ...
    context
        .log_info(
            &format!(
                "Skipping embedding search for {:?} (ML features not available)",
                request.terms
            ),
            "insights-api",
        )
...
async fn finalize_search_results(
    ...
    context
        .log_success(
            &format!(
                "Search completed: found {} results for {:?}",
                all_results.len(),
                request.terms
            ),
            "insights-api",

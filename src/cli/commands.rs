...
fn sort_insight_summaries(
    ...
    match sort {
        ...
        crate::server::types::SearchSort::Updated => {
            insights.sort_by(|a, b| b.updated_at.cmp(&a.updated_at));
        }
...
pub async fn update_insight(
    ...
    if overview.is_none() && details.is_none() {
        return Err(anyhow!(
            "At least one of --overview or --details must be specified"
        ));
    }
    ...
    let client = get_client();
    client
        .update_insight(topic, name, overview, details)
        .await?;
...
pub async fn delete_insight(topic: &str, name: &str, force: bool) -> Result<()> {
    ...
    match client.get_insight(topic, name, true).await {
        ...
        Err(_) => {
            ...
            Err(anyhow!("Insight {topic}/{name} not found"))
        }
    }
}
...
pub async fn index_insights(_force: bool) -> Result<()> {
    ...
    match client.reindex_insights().await {
        Ok(()) => {
            println!(
                "Re-indexing started. This will run in the background and may take some time."
            );
